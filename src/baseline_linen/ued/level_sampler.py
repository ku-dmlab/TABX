...
class LevelSampler:
    ...
    Args:
        ...
        staleness_coef (float, optional): The weighting factor for staleness. Defaults to 0.5.
    ...
    def initialize(self, pholder_level: Level, pholder_level_extra=None) -> Sampler:
        ...
        sampler = {
            "levels": jax.tree.map(
                lambda x: jnp.repeat(x[None], self.capacity, axis=0), pholder_level
            ),
        ...
        if pholder_level_extra is not None:
            sampler["levels_extra"] = jax.tree_util.tree_map(
                lambda x: jnp.array([x]).repeat(self.capacity, axis=0), pholder_level_extra
            )
        return sampler
    ...
    def insert_batch(
        self, sampler: Sampler, levels: Level, scores: chex.Array, level_extras: dict = None
    ...
    def find(self, sampler: Sampler, level: Level) -> int:
        ...
        eq_tree = jax.tree_util.tree_map(
            lambda X, y: (X == y).reshape(self.capacity, -1).all(axis=-1), sampler["levels"], level
        )
    ...
    def level_weights(
        ...
        return (1 - self.staleness_coef) * w_s + self.staleness_coef * w_c
    ...
    def score_weights(
        ...
        if prioritization == "rank":
            ...
            w_s = jax.nn.softmax(sampler["scores"], where=topk_mask)
    ...
    def _get_next_idx(self, sampler: Sampler) -> int:
        return jax.lax.select(
            sampler["size"] < self.capacity, sampler["size"], self.level_weights(sampler).argmin()
        )
