"""
Based on JaxMARL Implementation of MAPPO and sampling-for-learnability Implementation of SFL
"""
...
from src.tabx.wrappers import TABXAutoResetWrapper, TABXEnemyHeuristicWrapper, TABXLogWrapper
    ...
    def train(rng):
        ...
        def get_learnability_set(actor_train_state, rng):
            ...
            def _batch_step(unused, rng):
                rng, _rng, _rng_reset = jax.random.split(rng, 3)
                sample_rngs = jax.random.split(_rng, config["BATCH_SIZE"])
                levels = jax.vmap(sample_random_level, in_axes=(None, 0))(
                    init_env_params, sample_rngs
                )
                reset_rng = jax.random.split(_rng_reset, config["BATCH_SIZE"])
                obsv, env_state = jax.vmap(env.reset, in_axes=(0, 0))(reset_rng, levels)
                ...
                def _rollout(carry, unused):
                    ...
                    win = env_state["log_state"].returned_episode_wins[:, 0]

                    return carry, {"win": win, "done": done["__all__"]}
                ...
                carry, wins = jax.lax.scan(_rollout, carry, None, config["ROLLOUT_STEPS"])

                # Get win rate of each episode
                success_by_env = (
                    wins["win"].mean(axis=0, where=wins["done"]).reshape((1, config["BATCH_SIZE"]))
                )
                learnability_by_env = success_by_env * (1 - success_by_env).sum(axis=0)

                return None, (learnability_by_env, levels)

            rng, _rng = jax.random.split(rng)
            rngs = jax.random.split(_rng, config["NUM_BATCHES"])
            _, (learnability, levels) = jax.lax.scan(_batch_step, None, rngs, config["NUM_BATCHES"])

            flat_levels = jax.tree.map(lambda x: x.reshape((-1,) + x.shape[2:]), levels)
            learnability = learnability.flatten()
            top_k = jnp.argsort(learnability)[-config["NUM_TO_SAVE"] :]

            top_k_levels = jax.tree.map(lambda x: x.at[top_k].get(), flat_levels)

            return learnability.at[top_k].get(), top_k_levels

        # TRAIN LOOP
        def _update(update_runner_state, unused):
            runner_state, update_steps = update_runner_state
            train_states, env_state, last_obs, last_done, hstates, levels, rng = runner_state

            rng, _rng = jax.random.split(rng)
            learnability_scores, top_k_levels = get_learnability_set(runner_state[0][0], _rng)
            k_levels = top_k_levels["scenario"].healths.shape[0]
            ...
            def _update_step(update_runner_state, unused):
                ...
                def evaluate(actor_train_state, rng):
                    ...
                    def _rollout(carry, unused):
                        ...
                        action = pi.sample(seed=_rng)
                        env_act = unbatchify(
                            action, eval_env.agents, BATCH_SIZE, eval_env.num_agents
                        )
                        env_act = {k: v.squeeze() for k, v in env_act.items()}
                ...
                # Sample new envs
                rng, _rng, _rng_reset = jax.random.split(rng, 3)
                sample_rngs = jax.random.split(_rng, config["NUM_ENVS_TO_GENERATE"])
                new_levels = jax.vmap(sample_random_level, in_axes=(None, 0))(
                    init_env_params, sample_rngs
                )
                reset_rng = jax.random.split(_rng_reset, config["NUM_ENVS_TO_GENERATE"])
                obsv_gen, env_state_gen = jax.vmap(env.reset, in_axes=(0, 0))(reset_rng, new_levels)

                rng, _rng, _rng_reset = jax.random.split(rng, 3)
                sampled_level_idxs = jax.random.randint(
                    _rng, (config["NUM_ENVS_FROM_SAMPLED"],), 0, k_levels
                )
                sampled_levels = jax.tree.map(
                    lambda x: x.at[sampled_level_idxs].get(), top_k_levels
                )
                reset_rng = jax.random.split(_rng_reset, config["NUM_ENVS_FROM_SAMPLED"])
                obsv_sampled, env_state_sampled = jax.vmap(env.reset, in_axes=(0, 0))(
                    reset_rng, sampled_levels
                )

                levels = jax.tree.map(
                    lambda x, y: jnp.concatenate([x, y], axis=0), new_levels, sampled_levels
                )
                obsv = jax.tree.map(
                    lambda x, y: jnp.concatenate([x, y], axis=0), obsv_gen, obsv_sampled
                )
                env_state = jax.tree.map(
                    lambda x, y: jnp.concatenate([x, y], axis=0), env_state_gen, env_state_sampled
                )
            ...
            update_runner_state, metric = jax.lax.scan(
                _update_step, (runner_state, update_steps), None, config["UPDATE_FREQ"]
            )

            return update_runner_state, metric
        ...
        runner_state, metric = jax.lax.scan(_update, (runner_state, 0), None, config["NUM_UPDATES"])
