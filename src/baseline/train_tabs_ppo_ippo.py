...
class Config:
    ...
    critic_coef: float = 0.5  # The critic loss coefficient
...
if __name__ == "__main__":
    ...
    from src.baseline.algorithm import PPO, IPPO
    ...
    wandb.init(
        project="tabs_ppo_ippo", config=config, mode="online" if not config.debug else "disabled"
    )
    ...
    train_state_bs = ippo_bs.init_train_state(key_comb)
    ...
    def rollout_tabs(train_state_comb, train_state_deploy, train_state_bs, scenarios_comb):
        ...
        train_state_bs, rollout_result_bs = ippo_bs.rollout(train_state_bs, scenarios_bs)
    ...
    def train_bs(carry, _):
        ...
        train_state_bs, train_info_bs = ippo_bs.train(train_state_bs, rollout_result_bs)
    ...
    for step in tqdm(range(config.train_step // (config.log_step * config.n_env))):
        ...
        ippo_bs.save_state(train_state_bs, os.path.join(config.save_path, f"bs/ippo/{step}"))
