...
class Config:
    ...
    comb_ppo: PQNConfig = PQNConfig(rollout_step=tabs.max_n_ally, n_env=n_env, batch_size=32)
    deploy_ppo: PQNConfig = PQNConfig(rollout_step=tabs.max_n_ally, n_env=n_env, batch_size=32)
...
if __name__ == "__main__":
    ...
    # Agents
    ppo_unit_comb = PQN(config.comb_ppo, env_unit_comb)
    ppo_unit_deploy = PQN(config.deploy_ppo, env_unit_deploy)
    ...
    for step in tqdm(range(config.total_train_iter)):
        ...
        # Save models
        ppo_unit_comb.save_state(carry[0], os.path.join(config.save_path, f"comb/pqn/{step}"))
        ppo_unit_deploy.save_state(carry[1], os.path.join(config.save_path, f"deploy/pqn/{step}"))
