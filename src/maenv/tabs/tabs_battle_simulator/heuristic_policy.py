...
def heuristic_policy(
    ...
    healer_target = (
        (exist_injured_ally & injured_ally) | (~exist_injured_ally & is_ally)
    ) & is_healer
    # If the unit is not healer
    normal_target = ~is_ally & ~is_healer
    exist_attackable_target = (
        jnp.sum(observation[:, -2].astype(jnp.bool_) & (healer_target | ~is_healer)) > 0
    )
    ...
    masekd_distance = (
        L2_distnace + (~visible_target) * 1e6
    ...
    is_agressive = (
        masekd_distance < (attack_range * aggressive_threshold) ** 2
