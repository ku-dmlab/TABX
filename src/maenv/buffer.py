...
def make_buffer(data, max_length, key):
    data = jax.tree_util.tree_map(
        lambda value: jnp.repeat(value, max_length).reshape((max_length,) + value.shape), data
    )
...
def sample(dataset_state, batch_size):
    ...
    return dataset_state._replace(key=key), jax.tree_util.tree_map(
        lambda value: value[indices], data
    )
